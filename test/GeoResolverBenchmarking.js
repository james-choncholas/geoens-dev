...
contract('GeoENSResolver', async accounts => {
    ...
    it("should benchmark direct query resolution time", async () => {
        ...
        for (nt = 0; nt<num_tests; nt++) {
            ...
            for (nr=0; nr<num_requests; nr++) {
                ...
                a = await geoResolver.geoAddr(emptynode, web3.utils.fromAscii('ezs42bcd'), 8);
                t = process.hrtime(t);
    ...
    for (i = 0; i < 100; i++) {
        it("should resolve a 20km radius geohash search", async () => {
            ...
            var t = process.hrtime();
            a = await geoResolver.geoAddr(emptynode, web3.utils.fromAscii('ezs4'), 4);
            t = process.hrtime(t);
    ...
    for (i = 0; i < 100; i++) {
        it("should resolve a 630km radius geohash search", async () => {
            var t = process.hrtime();
            a = await geoResolver.geoAddr(emptynode, web3.utils.fromAscii('ez'), 2);
            t = process.hrtime(t);
